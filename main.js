...
class Products {
    async getProducts() {
        try {
            let result = await fetch("https://65cfdd8abdb50d5e5f5be980.mockapi.io/product");
        ...
        cartContent.addEventListener("click", event => {
            if (event.target.classList.contains("cart-remove-btn")) {
                ...
                if (tempItem.amount > 0) {
                    Storage.saveCart(cart);
                    this.saveCartValues(cart);
                    lowerAmount.nextElemenSibling.innerText = tempItem.amount;
                } else {
